...
def show_help():
    ...
    print("\nImportant Notes:")
    print("1. CA Certificate Import (OPTIONAL):")
    print("   - Only required when using as system proxy")
    print("   - To trust this CA on Windows:")
    print("     - Doule-click the certificate file and select 'Install Certificate'")
    print("     - Import to 'Trusted Root Certification Authorities'")
    print("2. SECURITY CRITICAL: This is a HTTPS proxy CA")
    print("   - It decrypts HTTPS traffic - NEVER share the certificate")
    print("   - HIGH RISK: Importing makes ALL HTTPS traffic vulnerable")
    print("   - MUST REMOVE from trusted certificates after each use")
    print("   - Use win + R to run 'certmgr.msc' and delete the certificate")
...
def main():
    ...
    if args.clear_cache:
        ...
        print("1. Windows Trust (OPTIONAL):")
        print("   - Only required for system proxy usage")
        print("   - Manually trust this CA certificate in Windows")
        print("2. SECURITY ALERT: This is a HTTPS proxy CA")
        print("   - NEVER share these files with anyone")
        print("   - HIGH RISK: Makes ALL HTTPS traffic vulnerable")
        print("   - MUST REMOVE after each use session")
        print("   - Use win + R to run 'certmgr.msc' and delete the certificate")
        print(f"     Certificate file: {CERT_FILE}")
        print(f"     Private key file: {KEY_FILE}")
    else:
